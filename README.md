# Data Analyst Portfolio

Welcome to my Data Analyst Portfolio. This repository contains links to my data analysis projects completed using Python, Pandas, Matplotlib, and Google Colab.

## Projects

### 1. Telco Customer Churn Analysis

Description:
Analyzed customer churn patterns to identify factors influencing customer retention.

Repository:
https://github.com/USERNAME/telco-customer-churn-analysis

### 2. Sales Performance Analysis

Description:
Analyzed sales and profit performance across product categories and regions using the Superstore dataset.

Repository:
https://github.com/USERNAME/sales-performance-analysis

## Tools & Technologies

* Python
* Pandas
* NumPy
* Matplotlib
* Google Colab
* GitHub

## Contact

GitHub: https://github.com/USERNAME
